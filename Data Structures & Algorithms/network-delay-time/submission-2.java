            ...
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                    dist[i][j] = Math.min(dist[i][j],
                                          dist[i][mid] + dist[mid][j]); // 比較是i -> j比較近；還是i -> m -> j比較近 (放距離近的那個)

        // 找出從 k-1 出發到所有點的最大距離（最慢收到訊號的節點）
        /*
        dist[k - 1]：取出從節點 k 出發的所有最短距離（第 k-1 列），因為陣列是從 0 開始的。
        Arrays.stream(...)：將這一列轉成 Java Stream。
        .max()：找出這一列中的最大值（也就是從節點 k 出發最遠的節點所需時間）。
        .getAsInt()：從 OptionalInt 取出實際的 int 值。
        */
